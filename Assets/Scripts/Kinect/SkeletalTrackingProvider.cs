    ...
    protected override void RunBackgroundThreadAsync(int id, CancellationToken token)
    {
        try
        {
            ...
            using (Device device = Device.Open(id))
            {
                ...
                //トラッキングの質を上げるには、ModelPathの引数に "dnn_model_2_0_op11.onnx" を入れる
                using (Tracker tracker = Tracker.Create(deviceCalibration, new TrackerConfiguration() { ProcessingMode = TrackerProcessingMode.Cuda, SensorOrientation = SensorOrientation.Default, ModelPath= "dnn_model_2_0_lite_op11.onnx" }))
                {
